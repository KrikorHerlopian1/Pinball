...
/*
	void do_intro (unsigned char *digit_images, unsigned char *letter_images, int lives, int score)
	{
	    int i;
		...
		draw_string(screen.xres/2 - 4*IM_WIDTH-IM_WIDTH/2, screen.yres/2, letter_images, "GET");
		draw_string(screen.xres/2 - IM_WIDTH/2, screen.yres/2, letter_images, "READY");
		usleep(FRAME_DELAY);
	    }
	}
*/
...
startLoopDoIntro:
	...
	ldr	R1, [R1]		@IM_WIDTH
	mov	R1, R1, LSR #1		@im_width/2
	sub	R0, R0, R1		@screen.xres / 2 - im_width/2
	ldr	R1, [R10, #4]		@screen.yres
	mov	R1, R1, LSR #1		@screen.yres / 2
	mov	R2, R7			@letter_images
	ldr	R3, =readyStr		@ready string.
	bl	draw_string
